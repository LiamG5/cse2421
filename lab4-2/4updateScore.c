	...
	if(traverse_ptr != NULL) {
		while(traverse_ptr != NULL) {
			if(traverse_ptr->Student.student_ID == ID) {
				printf("Recalculating grades for %s , Student ID Number: %d \n", traverse_ptr->Student.student_name, traverse_ptr->Student.student_ID);
				float scores[4][3];
				...
				printf("%s Cumlative: %.2f\n",CategoryNames, traverse_ptr->Student.Cat1.Cumulative);
				...
				printf("%s Cumlative: %.2f\n",(char*)CategoryNames+15, traverse_ptr->Student.Cat2.Cumulative);
				...
				printf("%s Cumlative: %.2f\n",(char*)CategoryNames+30, traverse_ptr->Student.Cat3.Cumulative);
				...
				printf("%s Cumlative: %.2f\n",(char*)CategoryNames+45, traverse_ptr->Student.Cat4.Cumulative);
